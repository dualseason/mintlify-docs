...
description: "一键部署和管理 OpenClaw AI 机器人网关的可视化工具，支持 Telegram、飞书、Discord 三大平台，通过 XCompute API 接入 GPT-5、Claude、Gemini 等多种 AI 模型。"
---
...
<Note>**Linux 远程访问：** 将 `127.0.0.1` 替换为服务器 IP，如 `http://your-server-ip:51942`</Note>
...
选择要使用的 AI 模型，并输入您的 XCompute API Key（以 `sk-` 开头）。
...
<Tip>
  **模型选择建议：** - 💰 **高性价比：** `claude-haiku-4-5-20251001`、`gemini-2.5-flash` - 🚀 **高性能：** `gpt-5`、`claude-opus-4-6-20260320`、`gemini-3-pro-preview` - ⚡
...
<CardGroup cols={3}>
  <Card title="Telegram" icon="telegram" href="/integrations/platform/openclaw-manager-telegram">
    通过 BotFather 创建 Telegram 机器人
  </Card>

  <Card title="飞书" icon="message" href="/integrations/platform/openclaw-manager-feishu">
    创建飞书企业自建应用机器人
  </Card>
...
<Note>**提示：** 对接码存储在 Gateway 内存中，重启后失效。如果对接码过期，让用户重新发送消息获取新的对接码即可。</Note>
...
<Card title="开始使用 XCompute" icon="rocket" href="https://XCompute.us/console">
  立即注册 XCompute，获取您的 API 密钥，部署您的 AI 机器人！
</Card>
