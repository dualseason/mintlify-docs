---
title: "XCompute — OpenAI 兼容 API 网关（GPT-5、Claude、Gemini）"
description: "OpenAI 兼容 API，支持 GPT-5、Claude、Gemini。多供应商路由、透明定价、低延迟。企业 SLA、SDK 支持、按需付费。"
---

统一的 OpenAI 兼容端点，支持 GPT-5、Claude 和 Gemini。只需将 Base URL 更改为 `https://XCompute.us/v1`，即可在几分钟内从 OpenAI 迁移——保留现有 SDK，无需重写代码。多供应商路由确保低延迟和高可用性。透明定价、企业 SLA 和全球 CDN 加速。

## 快速开始

<Card title="立即开始" icon="rocket" horizontal href="/api-reference/texts/general/chat-completions">
  OpenAI 兼容的聊天 API，支持 GPT-5、Claude Sonnet 4.5、Gemini 2.0。切换 Base URL，无需修改代码。
</Card>

## OpenAI 兼容端点

XCompute 提供 OpenAI 风格的端点，实现无缝迁移。只需更改 Base URL 并继续使用现有 SDK。所有接口遵循 OpenAI 标准，扩展支持 Claude、Gemini、Sora 和 VEO 模型。

### 聊天补全 API

支持 GPT-5、GPT-4o、Claude Sonnet 4.5、Gemini 2.0 Flash。OpenAI 兼容 API，支持流式传输和低延迟。

<Card title="聊天补全 API" icon="message" href="/api-reference/texts/general/chat-completions">
  支持流式传输和低延迟
</Card>
...
<Card title="图像生成 API" icon="image" href="/api-reference/images/gpt-image-1/generation">
  文本到图像生成
</Card>
...
<Card title="视频生成 API" icon="video" href="/api-reference/videos/veo3/generation">
  异步任务管理，支持 Webhook
</Card>
...
监控端点健康和性能。追踪图像和视频生成的异步任务进度，支持 Webhook 通知。[查看状态 →](/api-reference/tasks/status)
...
通过 `npm install openai` 安装。配置 baseURL 参数为 XCompute 端点，保留现有集成。
...
OpenAI 兼容的 Java 客户端。简单配置更改即可访问所有 XCompute 模型和供应商。

## 从 OpenAI 迁移

5 分钟内切换到 XCompute。无需重写代码——只需更改 Base URL 和 API Key。

<Steps>
  <Step title="获取 API Key">
    注册 XCompute 并从<a href="https://XCompute.us/console/token" rel="nofollow" target="_blank">控制台</a>获取 API Key。
  </Step>

  <Step title="更新 Base URL">
    将 OpenAI Base URL 改为 XCompute 端点。现有 SDK 集成无需修改即可工作。
  </Step>
...
<AccordionGroup>
  <Accordion title="如何从 OpenAI 迁移？">
    只需将 Base URL 更改为 `https://XCompute.us/v1` 并使用您的 XCompute API Key。保留现有 SDK——无需重写代码。迁移通常在 5 分钟内完成。
  </Accordion>
  ...
  <Accordion title="定价如何与 OpenAI 比较？">
    XCompute 提供透明的按 Token 定价，比官方供应商更实惠。按需付费无需订阅，企业用量享受批量折扣，无隐藏费用。
  </Accordion>
